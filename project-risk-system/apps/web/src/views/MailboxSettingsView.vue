        ...
        <div class="mailbox-layout">
          <form class="mailbox-form-card" novalidate @submit.prevent="save">
            ...
            <section class="form-section">
              ...
              <div class="rule-block"><div><strong>主题关键词 <em>*</em></strong><small>邮件主题包含任一关键词时进入周报分析</small></div><div class="keyword-editor"><div><span v-for="(item,index) in form.subjectKeywords" :key="item">{{ item }}<button type="button" :aria-label="`删除关键词 ${item}`" @click="removeKeyword(index)">×</button></span></div><label><input v-model="keywordInput" maxlength="20" placeholder="输入关键词后按回车" @keyup.enter.prevent="addKeyword"><button type="button" @click="addKeyword">添加</button></label></div></div>
          ...
          <aside class="mailbox-side">
            ...
            <section class="prototype-panel sync-flow-card"><p>DATA FLOW</p><h2>数据如何进入看板</h2><ol><li><b>1</b><span><strong>读取授权周报</strong><small>仅处理符合识别规则的新邮件</small></span></li><li><b>2</b><span><strong>项目名称匹配</strong><small>与已导入的标准项目清单关联</small></span></li><li><b>3</b><span><strong>AI 提取风险线索</strong><small>生成类别、等级和处置建议</small></span></li><li><b>4</b><span><strong>风险管理员确认</strong><small>低置信或歧义结果需要人工确认</small></span></li><li><b>5</b><span><strong>更新风险看板</strong><small>确认后进入周报汇总和风险清单</small></span></li></ol></section>
