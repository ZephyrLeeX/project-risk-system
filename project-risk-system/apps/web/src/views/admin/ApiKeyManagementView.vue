...
const form = reactive({ name: "", vendor: "OpenAI 兼容服务", endpoint: "", model: "", key: "", expiry: "", timeout: 60, retries: 2, enabled: true });
...
function openCreate(provider?: AiProviderListItem): void {
  ...
  Object.assign(form, provider ? {
    name: provider.name, vendor: provider.vendor, endpoint: provider.endpoint, model: provider.model,
    key: "", expiry: provider.expiresAt ?? "", timeout: provider.timeoutSeconds, retries: provider.retryCount, enabled: provider.enabled,
  } : { name: "", vendor: "OpenAI 兼容服务", endpoint: "", model: "", key: "", expiry: "", timeout: 60, retries: 2, enabled: true });
...
async function saveProvider(): Promise<void> {
  ...
  saving.value = true;
  try {
    const body = { name: form.name.trim(), vendor: form.vendor.trim(), endpoint: form.endpoint.trim(), model: form.model.trim(), expiresAt: form.expiry || null, timeoutSeconds: Number(form.timeout), retryCount: Number(form.retries), enabled: form.enabled };
    if (editingId.value) {
...
async function testDraft(): Promise<void> {
  ...
  testing.value = true;
  try {
    testResults.value = [await aiProviderApi.testDraft({ name: form.name.trim(), endpoint: form.endpoint.trim(), model: form.model.trim(), apiKey: form.key.trim(), timeoutSeconds: Number(form.timeout), retryCount: Number(form.retries) })];
...
<template>
  <AdminShell>
    ...
    <section class="prototype-panel"><header class="prototype-panel-heading"><div><p>CALL LOG</p><h2>最近调用状态</h2><span>只保留调用元数据与脱敏错误摘要</span></div><div class="segmented"><button :class="{active:callResult==='all'}" type="button" @click="selectCallResult('all')">全部</button><button :class="{active:callResult==='SUCCESS'}" type="button" @click="selectCallResult('SUCCESS')">成功</button><button :class="{active:callResult==='FAILURE'}" type="button" @click="selectCallResult('FAILURE')">失败</button></div></header><div class="admin-table-scroll"><table class="admin-table"><thead><tr><th>时间 / Trace ID</th><th>服务与模型</th><th>调用场景</th><th>Token</th><th>耗时</th><th>结果</th><th>错误摘要</th><th>操作</th></tr></thead><tbody><tr v-for="item in calls" :key="item.id"><td>{{ formatTime(item.createdAt) }}<small>{{ item.traceId.slice(0,8) }}…{{ item.traceId.slice(-4) }}</small></td><td>{{ item.providerName }}<small>{{ item.model }}</small></td><td>{{ sceneLabel(item.scene) }}</td><td>{{ item.totalTokens.toLocaleString('zh-CN') }}</td><td>{{ (item.durationMs/1000).toFixed(2) }}s</td><td><span class="status-pill" :class="item.result==='SUCCESS'?'status-active':'status-disabled'">{{ item.result==='SUCCESS'?'成功':'失败' }}</span></td><td>{{ item.errorSummary || '—' }}</td><td><button type="button" @click="openCallDetail(item.id)">详情</button></td></tr><tr v-if="!calls.length"><td colspan="8">暂无调用记录</td></tr></tbody></table></div><footer class="prototype-pagination"><span>显示第{{ callTotal ? (callPage-1)*pageSize+1 : 0 }}–{{ Math.min(callPage*pageSize,callTotal) }}条，共{{ callTotal }}条</span><div><button type="button" :disabled="callPage<=1" @click="changePage(callPage-1)">上一页</button><b>{{ callPage }}</b><button type="button" :disabled="callPage>=pageCount" @click="changePage(callPage+1)">下一页</button></div></footer></section>

    <ModalDialog v-if="drawerOpen" eyebrow="AI SERVICE CONFIGURATION" :title="editingId?'编辑 AI 服务':'新增 AI 服务'" @close="drawerOpen=false"><form class="prototype-form" autocomplete="off" @submit.prevent="saveProvider"><label><span>配置名称 *</span><input v-model="form.name" name="ai-service-display-name" autocomplete="off" required></label><label><span>服务商 *</span><select v-model="form.vendor" name="ai-service-vendor"><option>OpenAI 兼容服务</option><option>通用大模型服务</option></select></label><label class="full"><span>服务地址 *</span><input v-model="form.endpoint" name="ai-service-endpoint" autocomplete="url" required placeholder="https://api.example.com/v1"></label><label><span>模型名称 *</span><input v-model="form.model" name="ai-model-identifier" autocomplete="off" required></label><label><span>API Key {{ editingId?'（留空则不修改）':'*' }}</span><span class="credential-input"><input v-model="form.key" name="ai-service-api-key" :type="keyVisible?'text':'password'" autocomplete="new-password" spellcheck="false" :required="!editingId"><button type="button" @click="keyVisible=!keyVisible">{{ keyVisible?'隐藏':'显示' }}</button></span></label><label><span>有效期</span><input v-model="form.expiry" name="ai-key-expiry" type="date"></label><label><span>超时时间（秒）</span><input v-model="form.timeout" name="ai-timeout-seconds" type="number" min="1" max="300"></label><label><span>失败重试次数</span><input v-model="form.retries" name="ai-retry-count" type="number" min="0" max="5"></label><label class="switch-row full"><span><strong>启用此服务</strong><small>停用后不会参与模型调用</small></span><input v-model="form.enabled" name="ai-service-enabled" type="checkbox"></label></form><template #footer><button type="button" @click="drawerOpen=false">取消</button><button type="button" :disabled="testing" @click="testDraft">{{ testing?'测试中…':'保存前测试' }}</button><button class="admin-primary-button" type="button" :disabled="saving" @click="saveProvider">{{ saving?'保存中…':'保存配置' }}</button></template></ModalDialog>
