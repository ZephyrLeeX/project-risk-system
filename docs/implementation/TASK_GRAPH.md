...
| T035 | DESIGN_GAP (DG-16) | T031-T034, T040 | Define production Compose, Python processes, proxy, secrets and persistence after final backend/frontend composition. Wave 21 `IN_PROGRESS`; execution stop — Objective requires a "scheduler" component, but no approved periodic-trigger mechanism exists (design §4 has no scheduler box; ADR 0006 defines only Worker; ADR 0018 mandates outbox publisher + reconciler but leaves the trigger unspecified; T024 `schedule_enabled_syncs` has no driver). `publish_outbox`/`reconcile`/`schedule_enabled_syncs` have no production caller; no beat/scheduler entrypoint; `celery_app.py` (frozen T040) has no `beat_schedule`. Materializing a scheduler needs a new app entrypoint + cadence/single-active decisions and/or editing frozen `celery_app.py` (DESIGN_DEVIATION) — outside T035's Compose/proxy/Dockerfile/env write-set. No implementation, no review, no code checkpoint. See `reports/T035.md`, `reports/WAVE-21-PARTIAL.md`. |
...
| DG-16 | Production periodic-trigger / scheduler mechanism for the durable-task outbox publisher, lease reconciliation and scheduled mailbox sync. | T035, T037, T039 | ADR 0006 defines only a Celery Worker; design §4 has no scheduler box. ADR 0018 mandates an outbox publisher ("post-commit publish") and a Reconciler but does not specify the trigger (periodic process / worker-startup / inline) or cadence; T024 delivered `schedule_enabled_syncs` as a function with no production driver. `publish_outbox`/`reconcile`/`schedule_enabled_syncs` have no production caller, no beat/scheduler entrypoint exists, and `celery_app.py` (frozen T040 write-set) has no `beat_schedule`. T035 cannot deploy a scheduler within its Compose/proxy/Dockerfile/env write-set without a new app entrypoint + unapproved mechanism/cadence/single-active decisions or crossing the frozen `celery_app.py` write-set. |
