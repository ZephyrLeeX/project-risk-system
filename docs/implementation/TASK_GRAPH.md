...
| T033 | READY | T016, T032, T045 | Cut admin pages to Python APIs and remove fixed business states. T045 `REVIEW_PASSED` restored `_Contract` serialization-mode schema fidelity: frozen OpenAPI authority now expresses `AdminOverview`/`HealthItem`/`AttentionItem`/`RecentAuditItem`/`UnavailableSection` item fields with real types (no `unknown`). See `reports/T033.md`, `reports/T045.md`. |
| T034 | READY | T027, T030, T032, T043, T045 | Cut dashboard, weekly reports, mailbox and Agent UI to real Python APIs. T045 `REVIEW_PASSED` resolved the shared `_Contract` fidelity blocker — weekly_reports/agent schemas now express real field types in frozen OpenAPI authority. Not executed this run. |
