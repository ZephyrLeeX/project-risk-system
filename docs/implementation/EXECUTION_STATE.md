...
- Current Wave: Wave 12 IN_PROGRESS. T027 is BLOCKED (`DESIGN_GAP`: the approved sent/received fallback requires an immutable mail `received_at` fact that the current source/schema does not provide). T031 remains unstarted. Wave 11 PASS (T026/T042 REVIEW_PASSED; Integration PASS).
