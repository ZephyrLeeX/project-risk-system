...
- T030: `REVIEW_FAILED`. Candidate V2 category projection/confirmation work is unaccepted: PROCESS/RESOLVE bypass existing domain services and omit required timeline semantics; failure confirms lack metadata-only audit; pre-issue category staleness is not the approved retryable durable-task path; and required PostgreSQL confirmation/concurrency coverage is missing. No T030 checkpoint or Wave 15 Integration; Wave 15 remains `IN_PROGRESS`. T040, DG-05 and DG-08 remain untouched.
- T030 remediation review: `REVIEW_FAILED`. Domain mutation service reuse and transaction ownership were remediated, but the candidate still mismatches the ADR 0026 `RISK_CATEGORY_OPTIONS_V1` item field names, accepts explicit null category options for PROCESS/RESOLVE, and lacks sufficient durable category-stale retry/rebuild/exhaustion, legacy/API, strict concurrency and atomic rollback acceptance evidence. Remediation remains limited to T030; no checkpoint or Integration.
