...
class ReadOnlyAgentCore:
    ...
    def _system_instruction(
        ...
        instruction = (
            ...
            "金额、待办状态和周报数据都可能过期。用户使用‘继续/继续上一个问题/接着刚才的问题’"
            "等续问表达，且最近对话存在因取消或超时中断、尚未回答的系统领域问题时，应继续处理"
            "该未完成问题；若上一问题已完整回答，不要重复作答，应请用户说明要继续什么。不要仅"
            "因为续问没有重复‘项目/风险’关键词而拒绝。当前回答依赖业务事实时必须重新调用授权 tool，"
        ...
        instruction += (
            ...
            "时间范围新增风险查询 guidance：当用户询问本周/这周有哪些新增风险、上周/上星期"
            "新增风险、最近7天新增风险、本月/这个月新增风险、上个月新增风险等相对时间范围的"
            "新增风险时，必须优先调用 risk_list 并传 timeRange 预设，由服务端把相对时间解析为"
            "确定的上海时区时间区间——不要自行计算具体日期，不要把日期参数传给工具："
            "本周/这周→CURRENT_WEEK；上周/上星期→PREVIOUS_WEEK；最近7天/近7天→LAST_7_DAYS；"
            "本月/这个月→CURRENT_MONTH；上个月/上月→PREVIOUS_MONTH。"
            "如果同时指定了风险等级（如“这周新增了哪些高风险”），level 与 timeRange 一起传，"
            "例如 {\"level\": \"HIGH\", \"timeRange\": \"CURRENT_WEEK\"}。"
            "不得用 dashboard_summary、dashboard_focus 或 weekly_report 近似代替明确的新增风险"
            "查询（它们不按新增时间过滤，weekly_report 只覆盖邮件来源的周报口径）；只有用户明确"
            "询问周报或本周处理建议时才调用 weekly_report。注意区分两个意图："
            "“本周有哪些新增风险”→risk_list(timeRange=CURRENT_WEEK)；"
            "“给出本周处理建议”→weekly_report。risk_list 的时间过滤基于风险新增"
            "（detectedAt）时间，不是最后更新时间。"
            "新增风险的状态语义：“新增风险”是基于 detectedAt 的历史新增事实；"
            "用户只指定时间范围而未指定状态时，不要传 status，risk_list 会返回该"
            "时间范围内新增的全部风险（包括之后已 RESOLVED 的）；只有用户明确要求"
            "“未解决/当前仍存在/还在处理中”等状态语义时才传 status=ACTIVE，"
            "明确要求“已解决/已解除”时才传 status=RESOLVED。"
            "不要为普通的“本周新增风险”自动附加 {\"status\": \"ACTIVE\"}。"
